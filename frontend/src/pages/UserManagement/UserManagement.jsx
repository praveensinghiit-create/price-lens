...
const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    try {
      const res = await fetch("http://20.83.164.225:5000/api/users");
  ...
  const handleSave = async (e) => {
    ...
    try {
      const response = await fetch("http://20.83.164.225:5000/api/users", {
        method: "POST",
  ...
  const handleDelete = async (userId) => {
    ...
    try {
      const response = await fetch(
        `http://20.83.164.225:5000/api/users/${userId}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <div className="um-container">
      ...
      <form className="um-form" onSubmit={handleSave} autoComplete="off">
        ...
        <button type="button" className="um-cancel-btn" onClick={handleCancel}>
          CANCEL
        </button>
      </form>
